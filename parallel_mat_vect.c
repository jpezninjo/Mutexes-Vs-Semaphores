...
//  Print_vector("We read", local_x, local_n, my_rank, p, comm);
    ...
    printf("Process > %d, start time = %lf\n", my_rank, start_time);
    ...
    printf("Process > %d, runtime = %lf\n", my_rank, elapsed_time);

    MPI_Reduce(&elapsed_time, &total, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
    MPI_Barrier(MPI_COMM_WORLD);
    
    if(my_rank == 0) printf("Process > %d, Total time: %lf\n", my_rank, total);
...
void Parallel_matrix_vector_prod(
    ...
    // MPI_Reduce(&total_time, &total, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
    // //printf("Process > %d, process time = %lf", my_rank, total);
    // MPI_Barrier(MPI_COMM_WORLD);
    // //if(my_rank == 0)printf("Total time will print after everything else...\n");
    // //if(my_rank == 0){printf("Process > %d, Total time: %lf", my_rank, total_time);}
    // if(my_rank == 0){printf("Process > %d, Total time: %lf", my_rank, total);}
