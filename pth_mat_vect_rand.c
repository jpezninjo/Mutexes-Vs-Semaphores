...
void *Pth_mat_vect(void* rank) {
   ...
   //int anyones_last = local_m;
   int current_row = my_rank;

#  ifdef DEBUG
   printf("Thread %ld > my first index = %d, my last index = %d\n",
         my_rank, current_row, current_row*thread_count + n-1);
#  endif

   for (i = 0; i < local_m; i++) {
      y[current_row] = 0.0;
      for (j = 0; j < n; j++)
          y[current_row] += A[current_row*n+j]*x[j];
      current_row += thread_count;
   }
